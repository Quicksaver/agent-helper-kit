...
- **[tools] Description Normalization Boundary**: Trim both manifest descriptions when parsing and when deriving tool metadata so whitespace-only `userDescription` values fall back cleanly and padded manifest strings never leak into UI text. See `getContributedLanguageModelToolsFromManifest` in src/shellToolContracts.ts.
