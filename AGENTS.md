...
- **[approval] Fail-Closed Parsing**: Approval runs in `prepareInvocation` and must parse every subcommand, including newline-separated commands; ambiguous syntax, transient env-var prefixes, or conflicting case variants should all require explicit approval. See `analyzeShellRunRuleDisposition` in src/shellToolSecurity.ts.
...
- **[build] Recheck Bundling**: Keep `recheck` external in esbuild; it resolves helper worker paths via `require.resolve`, which breaks if bundled into the extension. See `extensionBuildOptions` in esbuild.mjs.
