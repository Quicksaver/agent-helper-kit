...
- **Test Script Coverage Updates**: `yarn test` runs Vitest with coverage in this repo, so raised coverage can update the reported global thresholds; that is expected and always acceptable. See `test` in package.json.
...
- **Browser Webview Coverage Scope**: Browser-only webview entries can live under `src/webviews` for linting/typechecking, but Vitest node coverage should exclude them unless a DOM harness executes them. See `coverage.exclude` in `vitest.config.ts`.
...
- **Vitest Query Import Typings**: Query-suffixed fresh imports can stabilize Vite module re-evaluation in tests, but `tsc` needs a matching ambient module declaration for those paths. See `importFreshShellToolContracts` in src/test/shellToolContracts.test.ts.
- **Manifest Test Isolation**: Full-suite Vitest runs can make `vi.doMock('node:fs')` plus query-suffixed reimports of `shellToolContracts.ts` flaky; prefer pure manifest/reader helpers for deterministic contract tests. See `buildShellToolMetadataFromReader` in src/shellToolContracts.ts.
- **Shell Auto-Approve Policy**: Keep shell approval in `prepareInvocation`, require both auto-approve settings gates, and fail closed when subcommand parsing is ambiguous. See `analyzeShellRunAutoApproval` in src/shellToolSecurity.ts.
- **Multiline Shell Parsing**: Treat bare `\n` and `\r\n` outside quotes as command separators during auto-approval analysis, or allowlisted first tokens can mask later denied subcommands. See `splitShellSubcommands` in src/shellToolSecurity.ts.
- **POSIX Single-Quote Parsing**: In shell subcommand splitting, handle single-quoted spans before backslash escaping so literals like `'a\'` do not fail closed. See `splitShellSubcommands` in src/shellToolSecurity.ts.
- **Double-Quoted Command Substitution**: POSIX shells still expand backticks and `$(` inside double quotes, so auto-approve parsing must reject those forms even within quoted spans. See `splitShellSubcommands` in src/shellToolSecurity.ts.
- **Regex Rule ReDoS Guard**: `recheck` sync analysis requires explicit flags and should fail closed on `vulnerable` or `unknown` results before auto-approve regex rules are compiled. See `parseRegexRule` in src/shellToolSecurity.ts.
- **Case-Variant Rule Conflicts**: Case-insensitive named auto-approve rules must fail closed when mixed-case variants disagree, or object insertion order can change approval outcomes for unmatched casing. See `getNamedRule` in src/shellToolSecurity.ts.
- **Recheck Bundling Boundary**: Keep `recheck` external in the extension esbuild bundle because it resolves helper worker paths at runtime via `require.resolve`. See `extensionBuildOptions` in esbuild.mjs.
- **Regex Validation Timeout Retry**: Configured auto-approve regex validation should retry one `recheck` timeout with a higher budget before failing closed, or environment-sensitive startup cost can incorrectly drop safe rules. See `validateConfiguredRegexRule` in src/shellToolSecurity.ts.
