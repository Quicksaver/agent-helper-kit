...
- **[2026-03-03] Incremental Output Cursor**: Stream-like reads are stateful per terminal; `custom_get_terminal_output` tracks a cursor and, after completion, the second read intentionally resets to full output once before resuming delta reads. See `readBackgroundOutput` in src/terminalRuntime.ts.
