...
- **Shell Completion Signal Source**: Treat the child process `exit` event as authoritative for completion and use `close` only as a fallback, otherwise inherited stdio can leave finished commands stuck running forever. See `completeBackgroundCommand` in src/shellRuntime.ts.
