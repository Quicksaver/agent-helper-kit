...
- **[shell] Git Non-Interactive Defaults**: Force `GIT_PAGER=cat`, `GIT_MERGE_AUTOEDIT=no`, and `GIT_EDITOR=:` in `ShellRuntime` so spawned git commands cannot hand control to a pager or editor prompt. See `buildShellEnv` in src/shellRuntime.ts.
