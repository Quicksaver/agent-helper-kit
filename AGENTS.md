...
- **Coverage Env Sanitization**: Vitest should standardize inherited shell env (`NODE_OPTIONS`, `TERM`, `COLORTERM`, `SHELL`, terminal dimensions, and color flags) before each test, or ambient terminal state can change `shellRuntime.ts` coverage without repo changes. See `src/test/setup.ts` in src/test/setup.ts.
