...
- **[2026-03-05] LM Tool Split Payloads**: `LanguageModelToolResult` can return multiple `LanguageModelDataPart` values, allowing terminal tools to emit YAML metadata (`application/yaml`) and raw output (`text/plain`) separately instead of markdown frontmatter wrapping. See `buildSplitOutputToolResult` in src/terminalTools.ts.
