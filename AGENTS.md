...
- **[2026-03-03] Run Tool Output Opt-In**: `run_in_terminal_enhanced` keeps command output opt-in (`full_output`/`last_lines`/`regex`); foreground still returns `id` plus status metadata by default and persists completed records so `get_terminal_output_enhanced` can read them later. See `customRunInTerminalTool` in src/terminalTools.ts.
